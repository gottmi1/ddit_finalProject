                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <input class="form-control" placeholder="일정명을 입력해 주세요." type="text" name="title" id="event-title" required <c:if test="${empInfo.empNo ne 1 }"> disabled </c:if> />
                            ...
                            <div class="col-12">
                            	<div class="mb-2">
                            		...
                            		<input class="form-control" type="datetime-local" name="start" id="event-start" required <c:if test="${empInfo.empNo ne 1 }"> disabled</c:if> />
                            ...
                            <div class="col-12">
                            	<div class="mb-2">
                            		...
                            		<input class="form-control" type="datetime-local" name="end" id="event-end" required <c:if test="${empInfo.empNo ne 1 }"> disabled </c:if> />
                            ...
                            <div class="col-6">
                            	<div class="mb-2">
                            		...
                            		<input type="checkbox" id="event-allday" value="Y" class="form-check-input" <c:if test="${empInfo.empNo ne 1 }"> disabled </c:if> />
                            		<div class="invalid-feedback">종료일을 입력해주세요</div>
                            	</div>
                            </div>
                            ...
                            <div class="col-3">
                                <div class="mb-3">
                                    <input type="color" name="textColor" id="event-category" class="color-input form-control" <c:if test="${empInfo.empNo ne 1 }"> disabled </c:if> />
                            ...
                            <div class="col-12">
                            	<div class="mb-2">
                            		...
                            		<textarea class="form-control" name="content" id="event-content" <c:if test="${empInfo.empNo ne 1 }"> disabled </c:if> ></textarea>
                            	</div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6">
                                <c:if test="${empInfo.empNo eq 1 }">
...
//         },
        select: function(info) {
        	if (${empInfo.empNo} != 1) {
                info.revert(); // empNo가 1이 아닐 때는 eventDrop 기능 비활성화
                return;
            }
...
//         	endDateObj.setMinutes(endDateObj.getMinutes() + 1);
        	...
        	let enddate;
       	    ...
       	    if (info.event.allDay) {
        		...
        		enddate = moment(info.event.end || info.event.start).format('YYYY-MM-DD');
       	        ...
       	        $.ajax({
       	        	url : "/commonCalendar/updateEventAllday.do",
       	        	type : "post",
	        	...
	        	$.ajax({
	        		url : "/commonCalendar/updateEvent.do",
	        		type : "post",
	        		contentType : 'application/json',
	        		data : JSON.stringify(myData),
	        		beforeSend : function(xhr){   // 데이터 전송 전, 헤더에 csrf 값 설정
        ...
        eventClick : function(info){
            ...
            $.ajax({
        		url : "/commonCalendar/eventDetail.do",
        		type : "post",
        		contentType : 'application/json',
...
//         		alert("리사이즈 취소");
    ...
    $(document).on('click','#btn-update-event',function(){
        ...
        $.ajax({
            url: '/commonCalendar/updateEventDetail.do',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
            	id : id,
    	 ...
    	 let id = $('#event-id').val();
    	    ...
    	    Swal.fire({
    	        ...
    	        if (result.isConfirmed) {
    	            ...
    	            $.ajax({
    	                url: '/commonCalendar/deleteEvent.do',
    	                method: 'post',
    	                contentType: 'application/json',
    	                data: JSON.stringify({
    	                    id: id
...
//             $('#event-end').val(endDateTime);
    ...
    $(document).on('click','#btn-save-event', function() {
        ...
        $('#event-modal').modal('hide'); // 모달 닫기
        $.ajax({
            url: '/commonCalendar/addEvent.do',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                title: title,
